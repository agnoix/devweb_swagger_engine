...
SwaggerResource.prototype.addOperations = function(resource_path, ops, consumes, produces) {
  if (ops) {
    ...
    for (var i = 0; i < ops.length; i++) {
      ...
      if(o["x-hidden"] && o["x-hidden"] == true) {
        continue; // Ayla custom flag to hide API, skips adding this Operation
      }
      ...
      // Ayla custom extensions
      var disableSandbox = false;
      if(o["x-disableSandbox"] && o["x-disableSandbox"] == true) {
        disableSandbox = true;
      }

      o.nickname = this.sanitize(o.nickname);
      op = new SwaggerOperation(o.nickname, resource_path, method, o.parameters, o.summary, o.notes, type, responseMessages, this, consumes, produces, o.authorizations, disableSandbox);
...
var SwaggerOperation = function(nickname, path, method, parameters, summary, notes, type, responseMessages, resource, consumes, produces, authorizations, disableSandbox) {
  var _this = this;
...
// res - adding asCurl to the model, similar to method used in later versions of swagger ui
// Note that asCurl exists on "request" already, however isn't easily accessible, and later versions of
// swagger-ui move it directly to the model, similar to what we are doing here
SwaggerOperation.prototype.asCurl = function(args, opts) {
  if (opts == null) {
    opts = {};
  }

  opts["mock"] = true; // force mock status

  obj = this["do"](args, opts, null, null); // Setup request as if it were to be run, but with mock

  var results = [];
  results.push('-X ' + obj.method.toUpperCase());
  ...
  return 'curl ' + (results.join(' ')) + ' \'' + obj.url.replace(/\'/g, '%27') + '\'';
};
  ...
  var transform = function(response) {
    ...
    if(contentType != null) {
      if(response.content.data && response.content.data !== "") {
        var trimData = response.content.data.trim();
        if(trimData == "") {
          out.data = null;
          out.obj = {};
          return out;
